...
| PR secret exposure | PR workflow reads E2E or provider secrets | Secretless `pull_request` workflows; separate trusted staging workflow |
...
As of Phase 0.5, GitHub branch protection/rulesets and Vercel Preview secret scopes require external owner remediation. Workflow code can reduce exposure, but it cannot compensate for an unprotected branch or Preview deployment with privileged credentials.
