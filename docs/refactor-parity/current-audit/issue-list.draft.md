...
- 2026-03-08 对最新预览 `https://graylum-ai-vercel-v1-3k58wgwsr-simons-projects-bfe3e99f.vercel.app` 的危险操作回归结果为：`admin-destructive 8/8`。
...
- 新站实际：当前已提供独立 `admin-destructive` 套件入口；在显式设置 `ENABLE_PARITY_DESTRUCTIVE_E2E=true` 时，已完成 `admin/settings` 对话历史清理、`admin/diagnostics` 旧记录清理、模型停用/恢复回滚、横幅公告发布/下线/恢复/删除回滚，以及积分包发布/下架/恢复/删除回滚的线上取证，但仍不会默认并入日常回归
