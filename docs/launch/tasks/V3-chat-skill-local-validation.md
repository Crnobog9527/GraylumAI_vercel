...
AgentKey 搜索接入单独交付：当前代码只具备已核验的 X 账号/帖子适配器，尚无可上线的通用搜索结果契约、Graylum 取数定价或真实许可验证。此次资料展示支持已有研究结果，不宣称发起过实时搜索。下一步需经 Owner 单独批准，以固定 MCP 目的地核对搜索目录、参数 schema、数值报价和结果格式，再接入受限搜索执行及自动关联；不得猜测工具名称、静默扩展白名单或把本地合成搜索当作实际可用。批准前不读取真实 Key、不调用真实接口、不改外部配置。
