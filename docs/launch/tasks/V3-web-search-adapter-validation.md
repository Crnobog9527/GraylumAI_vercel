...
### Chat reference UI and cancellation validation

The existing Skill reference section now accepts an explicit public-web query and displays returned source summaries, source links, and unknown publication dates. Results are adopted through the existing research-evidence command and can be associated with the step using its existing autosave. A scoped request identity is saved before dispatch; reload and explicit recovery retain that identity rather than issuing a fresh search. A synchronous in-flight guard prevents rapid clicks from replacing the recovery record.

Cancellation uses one service-only transaction for the original research transition and refund. Empty or prepared requests can be cancelled idempotently; dispatched work is retained and cannot be treated as unsent. Ordinary database users cannot invoke the cancellation RPC.

The first browser run exposed a cancellation failure after an empty intent was cancelled; this was fixed by returning the atomic cancellation outcome instead of rereading a cancelled plan. `/tmp/graylum-search-browser-validation-2.log`: 6 PASS / 91 SKIP, including real browser → local Next HTTP → Auth/PostgREST/SQL → official MCP loopback. The browser loses the first response after execution, reloads, recovers without a second provider execution or charge, displays and adopts sources, and saves step association. Disabled new dispatch and cancellation are also exercised. `/tmp/graylum-search-cancel-validation.log`: 1 PASS / 97 SKIP for empty/prepared/dispatched cancellation, repeated calls, balances and denied database access. Both runs pass cleanup and private-canary checks; migrations are reapplied. These use synthetic source content and a test-only loopback connector, not external search.

Reference renderer tests: 3 PASS (`/tmp/graylum-search-reference-unit.log`). Web typecheck exits 0 (`/tmp/graylum-search-cancel-types.log`); changed chat UI lint exits 0 (`/tmp/graylum-search-ui-lint.log`). No additional paid calls or external settings changes occurred. The task branch explicitly disables its automatic Vercel deployment, because migration 0071 is not externally applied.

Remaining: remote required checks, full candidate GitHub Codex Review and independent GPT-6/high audit; separately approved staging migration/configuration and external runtime acceptance. PR #389 is now merged into staging; its dual-model runtime acceptance is still separate from this local search validation. This does not approve a search price or activate the provider.


### PR 390 review remediation

Search adoption now recomputes the reviewed Tavily identity from the explicit query and original project/round/step before reading through the billed store or adopting any evidence. Another project owned by the same actor and another step are denied before settlement/adoption. Only the identity is passed to the existing artifact transaction; caller-supplied result/price remains forbidden.

An explicit provider tool error refunds the original user reservation idempotently; its provider cost remains unknown and the provider operation is not replayed. Unknown/dispatched operations retain their reservation. Once a failed request is durably refunded, the UI permits an explicit new search. Terminal recovery checks ownership and exact identity through a non-creating lookup before new-execution rate admission; a rate-limited new UUID creates no intent. Ordinary database callers cannot invoke the lookup.

The original tracked runner now applies 0071 twice and connects only the isolated copy to an official-SDK loopback fixture. `node packages/db/tests/v3/run-workbench.mjs --research-only` provides the focused reproducible search suite. The local MCP fixture supports separate sessions for multiple explicit searches. No production connector switch or new harness is added.

Latest focused validation: `/tmp/graylum-search-p1-regression-3.log`, 8 PASS / 91 SKIP, repeated migration and cleanup/private-canary PASS. Covers scoped adoption, settlement recovery after Skill deactivation, rate exhaustion recovery with no new intent, lookup access denial, closed-round browser recovery without adoption, provider failure refund and explicit new-search UI, and unknown retention. `/tmp/graylum-search-session-unit.log`: 58 PASS. Typecheck `/tmp/graylum-search-final-types.log`: exit 0 (before the final test-only assertion correction).

Earlier broad runs are not passes: `/tmp/graylum-search-tracked-chat-recovery.log` 48 PASS / 4 FAIL / 46 SKIP; all four fail in unscoped catalog fixture setup before their scenario assertions. `/tmp/graylum-search-recovery-admission.log` 78 PASS / 19 FAIL / 1 SKIP, with the same accumulated catalog setup failures, an obsolete candidate-error assertion, duplicate-project test setup, and a single-session MCP fixture unable to reconnect. Generation fixtures now use the existing module-scoped start; product timeout/permissions are unchanged. Follow-up regression for the prior failures is still pending. No actual provider search or remote migration occurred.
