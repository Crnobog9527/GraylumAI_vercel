...
/** Public result projection; writes belong to the executor atomic final commit. */
export function sliceResults(user:SupabaseClient,admin:SupabaseClient){
 async function read(input:z.infer<typeof scope>){
  ...
  const response=await admin.rpc('agent_slice_result',{p_actor_id:auth.data.user.id,p_execution_id:v.executionId,p_phase:v.phase,p_action:'read'}).abortSignal(AbortSignal.timeout(10000));
