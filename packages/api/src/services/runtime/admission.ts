...
export function runtimeAdmissionService(user:SupabaseClient,admin:SupabaseClient,policy:LocalRuntimePolicy){
 ...
 return {
  ...
  async prepare(value:unknown){
   ...
   const context={version:'runtime.v1',sdkVersion:'0.18.0',inputSelection:'scope-projection-v1',role:input.selection.kind==='auto'?'ordinary':input.selection.kind,input:input.input,instructions,model:row.data.model_id,
