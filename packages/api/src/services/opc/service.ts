    ...
    async prepareStep(value: unknown) {
      ...
      const organizerInstructions = v.purpose === "mentor" && v.organizeAfter
        ? "You are the independent structured-information extractor, separate from the public mentor. Return only one JSON object with this exact shape: {\"inputKind\":\"answer|acknowledgement|uncertainty|request|revision_request\",\"targetStepId\":\"an allowed step id\",\"informationPatch\":{\"allowed_field_id\":{\"value\":\"concise extracted value\",\"status\":\"provisional|unclear\",\"nature\":\"fact|decision|hypothesis|unknown\",\"basis\":\"user_statement|agent_proposal\"}}}. Extract only allowed fields. A user_fact value must be grounded in the user's latest statement and use basis user_statement. An agent_proposal value may come from the public mentor's concrete recommendation and uses basis agent_proposal. An uncertainty yields an empty patch. An acknowledgement or request must never be copied as a value. Never return confirmed or deferred. Preserve uncertainty and do not invent facts. " +
          "Each value must answer its allowed field with relevant business facts, decisions or constraints, not summarize the whole conversation. Separate business content from surrounding meta-instructions even within one sentence: exclude software verification labels, PR numbers, test-run provenance, and requests to save, retain, retry or manage this conversation. Leave that original wording in the conversation, not the field value. If a message supplies an answer mixed with these operational requests, use inputKind answer and extract only the supported business content; keep revision_request for an explicit change to another step. If there is no substantive answer, do not manufacture a user_fact patch from the operational request. " +
          ...
          "Examples (only when relevant to the current field): 调试记录：计划分享摄影练习，请保留聊天。 -> 计划分享摄影练习; 本轮验收：每周最多4小时，选题1小时、拍摄2小时、复盘1小时，请保留原请求。 -> 每周最多4小时，选题1小时、拍摄2小时、复盘1小时; 我的业务是软件测试，暂不商业化，先试运营一个月。 -> 我的业务是软件测试，暂不商业化，先试运营一个月。"
        : undefined;
