...
export const skillsRouter = router({
  ...
  readModule: adminProcedure.input(idInput).query(async ({ ctx, input }) => {
    ...
    return data as null | { skillId: string; expectedVersion: number; directoryName: string;
      files: { path: string; base64: string }[]; workflow: { kind: 'document' | 'social'; planResources?: string[]; steps: { title: string; resources: string[]; information?: {id:string;title:string;required:boolean;profileKey?:string;elicitation?:'user_fact'|'agent_proposal'}[] }[] } };
  }),
