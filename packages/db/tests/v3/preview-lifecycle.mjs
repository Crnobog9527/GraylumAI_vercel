...
export function signPreviewJwt(
  secret,
  role,
  { nowSeconds = Math.floor(Date.now() / 1000), ttlSeconds = PREVIEW_TOKEN_TTL_SECONDS } = {},
) {
  ...
  if (!Number.isInteger(ttlSeconds) || ttlSeconds <= 0 || ttlSeconds > PREVIEW_TOKEN_TTL_SECONDS)
...
export function parsePreviewOptions(args) {
  ...
  if (action === "destroy" && confirmDestroy !== id)
    throw new Error("destroy requires --confirm-destroy=<same-preview-id>");
  if (action !== "destroy" && confirmDestroy)
    throw new Error("--confirm-destroy is only valid for destroy");
...
export function previewDecision(options) {
  if (!options.persistent)
    return { bootstrap: true, runTests: true, controlOnly: false, destroyBackendsOnFinally: true };
  if (options.action === "stop" || options.action === "destroy")
    return { bootstrap: false, runTests: false, controlOnly: true, destroyBackendsOnFinally: false };
  return {
...
export function previewStatePath(id, env = process.env) {
  if (!SAFE_PREVIEW_ID.test(id)) throw new Error("invalid preview id");
  const root = env.V3_PREVIEW_STATE_ROOT
    ? resolve(env.V3_PREVIEW_STATE_ROOT)
    : resolve(homedir(), ".graylum", "workbench-previews");
  return { root, file: resolve(root, `${id}.json`), evidence: resolve(root, `${id}-evidence`) };
}
...
function assertStateShape(value, id) {
  ...
  if (
    ...
    !Array.isArray(value?.structuralArgs) ||
    value.names?.tag !== names.tag ||
    value.names?.db !== names.db ||
    value.names?.rest !== names.rest ||
    value.names?.auth !== names.auth ||
    value.names?.volume !== names.volume
  ...
  return args
    .filter(
      (arg) =>
        ...
        !arg.startsWith("--confirm-destroy=") &&
        !arg.startsWith("--case-pattern="),
    )
    .sort();
}
...
export function validateResumeState(state, expectedArgs) {
  if (!state.initialized)
    throw new Error("PREVIEW_NOT_INITIALIZED; destroy explicitly before starting over");
  if (JSON.stringify(state.structuralArgs) !== JSON.stringify([...expectedArgs].sort()))
    throw new Error("PREVIEW_ARGUMENT_MISMATCH");
  return state;
}
