...
### Phase 10 安全与合规审计 ✅ 已完成 (2026-01-21)
...
| # | 类别 | 检查项 | 状态 | 结果 |
|---|------|--------|------|------|
| 1 | 计费安全 | 后端计算流程 | ✅ | 预扣→结算→退费原子化 |
| 2 | 计费安全 | 配置对齐 | ⚠️ P1 | 硬编码费率，需读取 models 表 |
| 3 | 计费安全 | 逻辑严密性 | ⚠️ P1 | settle() 缺 cost 校验 |
| 4 | 代码规范 | 代码一致性 | ✅ | Drizzle + tRPC 规范 |
| 5 | 数据层 | 数据共享 | ✅ | 同 Supabase 实例 |
| 6 | 前端功能 | Header 积分显示 | ❌ P0 | useState(100) 硬编码 |
| 7 | 前端功能 | Sidebar 对话切换 | ⚠️ P1 | 切换时取消旧请求 |
| 8 | 前端功能 | Next.js 路由系统 | ✅ | App Router 正确使用 |
| 9 | API 安全 | 权限/速率限制 | ✅ | protectedProcedure + 60/min |
| 10 | 计费反作弊 | 熔断/隔离 | ✅ | 10K/hour 熔断 |
| 11 | 内容合规 | 审查/注入防御 | ✅ | contentModerator 10+ 规则 |
| 12 | 数据隐私 | RLS 多租户 | ❌ P0 | 仅 3/18 表有 RLS |
| 13 | 环境安全 | CORS/.env | ✅ | allowedOrigins 白名单 |
| 14 | 事务安全 | 行级锁/预扣 | ❌ P0 | 乐观锁非原子性 |
| 15 | AI 优化 | 智能路由 | ✅ | 任务分类器完整 |
| 16 | AI 优化 | 上下文压缩 | ✅ | 滑动窗口 + cache_control |
| 17 | 可观测性 | 幂等性/对账 | ❌ P0 | 缺 idempotencyKey |
| 18 | 前端交互 | 流式中断结算 | ❌ P0 | 中断未触发 settle() |

#### 问题分布

| 优先级 | 数量 | 描述 |
|--------|------|------|
| **P0 紧急** | 6 | Header积分、RLS缺失、事务原子性、幂等性、流式中断、settle校验 |
| **P1 重要** | 7 | 费率硬编码、对话切换、请求签名、Service Role Key 等 |
| **通过** | 11 | 后端计费、权限控制、内容审核、智能路由、上下文压缩 |

---

### Phase 11 安全审计问题修复 ⬜ 规划完成 (2026-01-21)

**目标**: 修复 Phase 10 审计发现的 P0/P1 问题
...
| 4 | 数据库事务原子性 | `billing.ts` | 使用 `db.transaction()` 包装 |
| 5 | 流式中断积分结算 | `useAIChat.ts`, `ai.ts` | 中断时发送 FINISH 事件触发 settle |
| 6 | settle() 成本校验 | `billing.ts:229` | 添加 `actualCost <= estimatedCost * 1.2` 校验 |

**11.2 P1 重要改进** (7项)

| # | 任务 | 位置 | 交付物 |
|---|------|------|--------|
| 1 | 费率动态读取 | `tokenCounter.ts` | 从 models 表实时读取 |
| 2 | 对话切换取消请求 | `ChatSidebar.tsx` | 切换时调用 abort() |
| 3 | 请求签名验证 | `securityChecks.ts` | HMAC 签名中间件 |
| 4 | Service Role Key 隔离 | `billing.ts` | 仅计费服务使用 admin client |
| 5 | 审计日志增强 | `billing.ts` | 添加 ip, ua, requestId |
| 6 | 自动对账任务 | 新增 | 每日积分/账单校对 cron |
| 7 | 错误监控集成 | 新增 | Sentry 或 LogRocket 集成 |
