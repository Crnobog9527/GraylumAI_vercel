...
export function useScrollPosition(key:string|null,ready:boolean){
 const ref=useRef<HTMLDivElement>(null),restored=useRef<string|null>(null),touched=useRef(false);
 useLayoutEffect(()=>{restored.current=null;touched.current=false;},[key]);
 useLayoutEffect(()=>{
