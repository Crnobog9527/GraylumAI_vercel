...
export function StrategyOverviewDialog({account,onClose,onSaved}:{account:Account;onClose:()=>void;onSaved:()=>Promise<unknown>}){
 ...
 function beginEdit(){const schema=latestSchema.data as Schema|undefined;if(!schema||data?.draftId!==draftId){setError('当前定位问题暂时无法读取，请稍后重试。');return;}const next:Edits={};for(const step of schema.workflow.steps){next[step.id]={};const values=(data?.information as Record<string,{values:Record<string,Value>|null}>|undefined)?.[step.id]?.values??null;for(const field of step.information??[])next[step.id][field.id]=baseValue({values},field);}const versions=data?.snapshot?.steps as Record<string,{version:number}>|undefined;if(account.pendingStrategyDraftId&&schema.workflow.steps.some(step=>!Number.isInteger(versions?.[step.id]?.version))){setError('当前定位版本暂时无法读取，请重新打开后再修改。');return;}setStepVersions(account.pendingStrategyDraftId?Object.fromEntries(schema.workflow.steps.map(step=>[step.id,versions![step.id].version])):null);setEditSchema(schema);setEdits(next);setError('');setSaved('');setEditing(true);}
 async function save(){
  ...
  try{await navigator.locks.request(key,async()=>{
   ...
   setSyncFailed(!refreshed);setEditing(false);setEditSchema(null);setSaved('已保存到此账号的待确认定位草稿。原正式版本及已有选题、稿件来源保持不变；请逐项确认后再发布新版。'+(refreshed?'':'页面暂未刷新，请重新打开定位详情核对。'));
  });}
 ...
 return <div className={styles.backdrop} onMouseDown={event=>{if(event.target===event.currentTarget&&!saving)onClose();}}><section role="dialog" aria-modal="true" aria-label="定位详情" className={editing?`${styles.dialog} ${styles.editDialog}`:styles.dialog}>
  ...
  <footer>{history?<button type="button" onClick={()=>setHistory(false)}>返回当前定位</button>:editing?<><button type="button" disabled={saving} onClick={()=>{setEditing(false);setEditSchema(null);setError('');}}>取消</button><button type="button" className={styles.primary} disabled={saving||(!changed&&!recoverable)} onClick={save}>{saving?'保存中…':recoverable?'恢复原保存':'确认保存'}</button></>:<><button type="button" disabled={syncFailed||read.isFetching||data?.draftId!==draftId||!latestSchema.data||latestSchema.isLoading} onClick={beginEdit}>修改定位</button><button type="button" onClick={()=>setHistory(true)}>历史版本</button><Link className={styles.primary} href={'/positioning/'+(savedDraftId??draftId)}>回到策略讨论</Link></>}</footer>
 </section></div>;
}
