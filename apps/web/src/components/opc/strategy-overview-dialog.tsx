...
type Schema={registrationId:string;workflow:{steps:Step[]}};
type Edits=Record<string,Record<string,string>>;
type Frozen={accountProjectId:string;requestId:string;expectedSourceVersionId:string;expectedPendingDraftId:string|null;expectedRegistrationId?:string|null;edits:Edits};
...
export function StrategyOverviewDialog({account,onClose,onSaved}:{account:Account;onClose:()=>void;onSaved:()=>Promise<unknown>}){
 ...
 const [editing,setEditing]=useState(false),[editSchema,setEditSchema]=useState<Schema|null>(null),[edits,setEdits]=useState<Edits>({}),[saving,setSaving]=useState(false),[error,setError]=useState(''),[saved,setSaved]=useState(''),[recoverable,setRecoverable]=useState(false),[savedDraftId,setSavedDraftId]=useState<string|null>(null);
 ...
 function beginEdit(){const schema=latestSchema.data as Schema|undefined;if(!schema){setError('新版定位问题暂时无法读取，请稍后重试。');return;}const next:Edits={};for(const step of schema.workflow.steps){next[step.id]={};const values=(data?.information as Record<string,{values:Record<string,Value>|null}>|undefined)?.[step.id]?.values??null;for(const field of step.information??[])next[step.id][field.id]=baseValue({values},field);}setEditSchema(schema);setEdits(next);setError('');setSaved('');setEditing(true);}
 async function save(){
  ...
  try{await navigator.locks.request(key,async()=>{
   ...
   if(raw)frozen=JSON.parse(raw) as Frozen;
    ...
    frozen={accountProjectId:account.projectId,requestId:crypto.randomUUID(),expectedSourceVersionId:account.sourceVersionId,expectedPendingDraftId:account.pendingStrategyDraftId??null,expectedRegistrationId:editSchema.registrationId,edits:changes};
 ...
 return <div className={styles.backdrop} onMouseDown={event=>{if(event.target===event.currentTarget&&!saving)onClose();}}><section role="dialog" aria-modal="true" aria-label="定位详情" className={editing?`${styles.dialog} ${styles.editDialog}`:styles.dialog}>
  ...
  <footer>{history?<button type="button" onClick={()=>setHistory(false)}>返回当前定位</button>:editing?<><button type="button" disabled={saving} onClick={()=>{setEditing(false);setEditSchema(null);setError('');}}>取消</button><button type="button" className={styles.primary} disabled={saving||(!changed&&!recoverable)} onClick={save}>{saving?'保存中…':recoverable?'恢复原保存':'确认保存'}</button></>:<><button type="button" disabled={!latestSchema.data||latestSchema.isLoading} onClick={beginEdit}>修改定位</button><button type="button" onClick={()=>setHistory(true)}>历史版本</button><Link className={styles.primary} href={'/positioning/'+(savedDraftId??draftId)}>回到策略讨论</Link></>}</footer>
 </section></div>;
}
