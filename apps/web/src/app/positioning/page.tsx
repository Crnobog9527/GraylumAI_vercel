...
import { CalendarDays, ChartNoAxesColumnIncreasing, Sparkles, Target } from 'lucide-react';
...
export default function PositioningHome() {
  ...
  function chooseTopics(){setIntent('topics');setShowStart(false);if(!startInput.trim()){const value='请基于当前账号定位，帮我策划下一批选题。';setStartInput(value);sessionStorage.setItem('opc-new-task-input',value);}}
  function choosePosition(){setIntent('');setShowStart(true);if(!startInput.trim()){const value='我想梳理一个账号的定位，请从需要确认的问题开始。';setStartInput(value);sessionStorage.setItem('opc-new-task-input',value);}}
  return (
    <WorkspaceFrame area="start"><main className={styles.page}><header className={styles.startTop}><strong>我的增长工作</strong>{workReturn&&<Link href={workReturn}>返回当前工作 →</Link>}</header><div className={styles.content}>
      ...
      <section className={styles.actions} aria-label="选择新任务">
        ...
        <button aria-pressed={showStart} onClick={choosePosition}><Target size={15}/>梳理账号定位</button>
      ...
      {!intent&&!showStart&&<section className={styles.discovery}><div className={styles.discoveryHead}><div className={styles.recommendTabs}>{([['all','为你推荐'],['strategy','策略'],['writing','写作'],['operations','运营']] as const).map(([id,label])=><button key={id} aria-pressed={recommendation===id} onClick={()=>setRecommendation(id)}>{label}</button>)}</div><Link href="/workbench/marketplace">浏览全部 →</Link></div><div className={styles.discoverGrid}>
