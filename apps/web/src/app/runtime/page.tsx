...
export default function RuntimePage(){
 ...
 const saveContent=trpc.opc.saveContentResult.useMutation(),prepareVideoMaterial=trpc.opc.prepareVideoMaterial.useMutation(),checkVideo=trpc.opc.checkVideoExecution.useMutation(),savePackage=trpc.opc.saveVideoPackage.useMutation();
 ...
 async function runVideo(initial:VideoOperation){
  ...
  try{await navigator.locks.request(videoKey,async()=>{
   ...
   await savePackage.mutateAsync({workItemId:op.workItemId,requestId:op.package.requestId,executionId:packageExecutionId,sourceScriptId,expectedStoryboardVersion:op.package.expectedStoryboardVersion,expectedEditingVersion:op.package.expectedEditingVersion});
 ...
 async function finalizeAndContinue(executionId:string){
  if(!workItem||!choices.data)return;const frozen=videoKey?localStorage.getItem(videoKey):null;if(frozen){await runVideo(JSON.parse(frozen));return;}const rejectedPackage=videoKey?localStorage.getItem(videoKey+':rejected-source:'+executionId):null;const rejectedRaw=videoKey&&rejectedPackage?localStorage.getItem(videoKey+':rejected:'+rejectedPackage):null;const rejected:VideoOperation|undefined=rejectedRaw?JSON.parse(rejectedRaw):undefined;const rejectedSelection=rejected&&rejected.workItemId===workItem.workItemId&&rejected.script.executionId===executionId?rejected.followup.selection:null;const skill=choices.data.skills.find(s=>'skill:'+s.moduleId===activeSelection);const selection=rejectedSelection??(skill?{kind:'skill' as const,moduleId:skill.moduleId,revisionId:skill.revisionId}:null);if(!selection){setError('请选择当前选题可用的 Skill，再定稿口播稿。');return;}
  const existing=versions.find(v=>v.kind==='script'&&v.status==='final'&&v.executionId===executionId);
  const retried=Boolean(videoKey&&localStorage.getItem(videoKey+':rejected-source:'+executionId)),followupId=retried?crypto.randomUUID():executionId;
  const op:VideoOperation={workItemId:workItem.workItemId,script:{requestId:existing?.requestId??executionId,executionId,expectedVersion:existing?existing.version-1:latest('script')},followup:{requestId:followupId,input:'[OPC_VIDEO_PACKAGE_V1] 基于资料中已定稿的口播稿，直接给出对应分镜和剪辑建议。只返回严格 JSON 对象，且只含 storyboard 与 editing 两个字符串字段；不要生成图片、视频或执行发布。',selection},package:{requestId:followupId,expectedStoryboardVersion:latest('storyboard'),expectedEditingVersion:latest('editing')},...(existing?{sourceScriptId:existing.id}:{})};await runVideo(op);
 }
