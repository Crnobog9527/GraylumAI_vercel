 ...
 return <main className="flex h-dvh min-h-0 flex-col bg-[var(--bg-primary)] text-[var(--text-primary)]">
  ...
  <div className="min-h-0 flex-1 overflow-y-auto" aria-label="对话记录">
   ...
   <section className="mx-auto w-full max-w-4xl space-y-6 p-4 sm:p-6">{executions?.map(e=><article key={e.executionId} className="space-y-4">
    {e.input&&<div className="flex justify-end gap-3"><p className="max-w-[85%] whitespace-pre-wrap break-words rounded-2xl rounded-br-sm bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] px-4 py-3 text-[var(--bg-primary)]">{e.input.replace(/^\[OPC_SCRIPT_V1\]\s*/, '')}</p><User className="mt-3 h-5 w-5 shrink-0 text-[var(--text-secondary)]"/></div>}
