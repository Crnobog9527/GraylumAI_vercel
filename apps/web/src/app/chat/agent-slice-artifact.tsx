...
export function SliceArtifact({target,works,candidate,onChanged,onContinue}:{target:SliceTarget;works:SliceWork[];candidate?:{candidateId:string;body:string;adoptable:boolean};onChanged:()=>void;onContinue:(projectId:string,roundId:string,pairId:string)=>Promise<void>}){
 ...
 const identity=target.projectId+':'+target.roundId;const activeIdentity=useRef(identity);activeIdentity.current=identity;
 const current=()=>activeIdentity.current===identity;
 useEffect(()=>{activeIdentity.current=identity;return()=>{activeIdentity.current='';};},[identity]);
 const showReport=async()=>{const value=await utils.client.workbench.report.query(scope);if(current())setReport(value);};
 ...
 const action=async(fn:()=>Promise<unknown>)=>{if(locked.current)return;locked.current=true;setBusy(true);setError('');try{await fn();if(current()){await query.refetch();if(current())onChanged();}}catch{if(current())setError('操作未完成。请重新读取状态后重试；你的编辑仍保留。');}finally{locked.current=false;setBusy(false);}};
 ...
 const publish=()=>{if(!query.data)return;const payload={action:'publish' as const,...scope,expectedSteps:Object.fromEntries(Object.entries(query.data.steps).map(([id,s])=>[id,{version:s.version,reviewVersion:s.reviewVersion}]))};void action(async()=>{await send({...payload,requestId:requestId(payload)});await showReport();});};
 const continuation=()=>void action(async()=>{
  const formal=await utils.client.workbench.report.query(scope);if(!current())return;if(!formal.available||!formal.id)throw new Error('unavailable');
  ...
  if(current())await onContinue(next.projectId,next.roundId,target.pairId);
 });
 return <aside aria-busy={busy} aria-label="当前作品成果" className="w-full space-y-4 border-l border-[var(--border-primary)] bg-[var(--bg-primary)] p-4 lg:w-80 lg:shrink-0 lg:overflow-y-auto">
 ...
 {query.data?.state==='published'&&<section className="space-y-3"><p>正式版本已保存</p><Button variant="outline" disabled={busy} onClick={()=>void action(showReport)}>查看正式报告</Button>
 ...
 <Dialog open={!!report} onOpenChange={open=>{if(!open)setReport(null);}}><DialogContent aria-describedby={undefined} className="max-h-[85dvh] max-w-4xl overflow-y-auto"><DialogTitle>{report?.report?.title??'正式报告'}</DialogTitle>{report?.available?report.report?.sections.map(section=><section key={section.stepId} className="space-y-3"><h3 className="font-semibold">{section.title}</h3><p className="whitespace-pre-wrap break-words leading-7">{section.body}</p></section>):<p>这份报告目前不可访问。</p>}</DialogContent></Dialog>
 </aside>;
}
