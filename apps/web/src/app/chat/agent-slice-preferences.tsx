...
export function SlicePreferences({account}:{account?:string|null}){
 ...
 {account&&<label><input type="checkbox" disabled={change.isPending} checked={accountOnly} onChange={e=>{setAccountOnly(e.target.checked);setValue('');}}/> 仅用于当前账号</label>}
 ...
 <textarea aria-label="写作偏好" disabled={change.isPending} value={value} maxLength={1000} onChange={e=>setValue(e.target.value)} placeholder="例如：口播稿使用简洁中文，先给具体例子。" className="w-full rounded-lg bg-[var(--bg-secondary)] p-2"/>
 <Button type="button" disabled={!value.trim()||change.isPending||!query.data} onClick={()=>void save('confirm')}>确认保存偏好</Button>
 {current?.active&&<Button type="button" variant="outline" disabled={change.isPending} onClick={()=>void save('delete')}>删除这条偏好</Button>}
