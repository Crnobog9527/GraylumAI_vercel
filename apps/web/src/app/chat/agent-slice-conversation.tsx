...
export function AgentSliceConversation({conversationId,navigate}:{conversationId:string;navigate:(id?:string)=>void}){
 ...
 {turns.map(turn=>{const owner=(targets.data??[]).find(work=>work.projectId===turn.projectId&&work.roundId===turn.roundId&&work.pairId===turn.pairId);return <section key={turn.executionId} aria-label="一轮对话" className="space-y-5">{turn.input!==null&&<div className="ml-auto w-fit max-w-[85%] whitespace-pre-wrap break-words rounded-2xl bg-[var(--color-primary)] px-4 py-3 text-black">{turn.input}</div>}<div aria-label="助手回答" className="min-w-0 space-y-3">
 <p aria-label="本轮回答归属" className="text-sm text-[var(--text-secondary)]">{owner?`${owner.title} · ${turn.stepTitle}`:`作品信息暂不可用 · ${turn.stepTitle}`}</p>
 ...
 {target&&<p aria-label="当前创作目标">{target.state==='published'?'正在查看：':'正在修改：'}{target.title} · {target.state==='published'?`正式 v${target.version}`:'当前草稿'} · {target.stepTitle}</p>}
