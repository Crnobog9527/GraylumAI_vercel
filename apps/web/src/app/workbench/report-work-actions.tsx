...
export function ReportWorkActions({report}:{report:ArtifactReport}) {
  ...
  const locked=useRef(false),scope=useRef(0);
  useEffect(()=>{
    ...
    return ()=>{current=false;scope.current++;};
...
export function WorkSource({projectId,roundId,title,canRevise,onRevised}:{projectId:string;roundId:string;title:string;canRevise:boolean;onRevised:(roundId:string)=>Promise<void>}) {
  ...
  const pending=useRef<Parameters<typeof api.createWork.mutate>[0]|null>(null),lock=useRef(false),scope=useRef(0);
  useEffect(()=>{let current=true;setSource(null);setVersions([]);setText('正在读取定位来源…');pending.current=null;
    ...
    }).catch(()=>{if(current)setText('定位来源暂不可用，相关内容不可使用。');});return()=>{current=false;scope.current++;};
  },[api,projectId,roundId]);
  return <section className="my-3 space-y-2 text-sm" aria-label="作品定位来源"><p aria-live="polite">{text}</p>
    {source&&canRevise&&<>
      <label>修订所用定位版本<select aria-label="修订所用定位版本" className="mt-2 block max-w-full bg-zinc-900" disabled={busy} value={selected} onChange={e=>{setSelected(e.target.value);pending.current=null;}}>{versions.map(v=><option key={v.roundId} value={v.roundId}>定位 v{v.version}</option>)}</select></label>
      <p>新修订不会修改已保存版本；采用不同定位版本会从空工作稿开始。</p>
      <Button disabled={busy} onClick={()=>{void(async()=>{
        if(lock.current)return;lock.current=true;const scopeId=scope.current;setBusy(true);
        try{
          if(!pending.current){const report=await api.report.query({projectId:source.sourceProjectId,roundId:selected});if(scopeId!==scope.current)return;if(!report.available||!report.id)throw new Error();const id=crypto.randomUUID();pending.current={projectId,roundId:id,requestId:id,fromRoundId:roundId,sourceVersionId:report.id,configId:source.configId,title};}
          const created=await api.createWork.mutate(pending.current);if(scopeId===scope.current)await onRevised(created.roundId);
        }catch{if(scopeId===scope.current)setText('修订暂未完成，请重试；原作品已保留。');}finally{lock.current=false;setBusy(false);}
      })();}}>创建本作品的新修订</Button>
    </>}
  </section>;
}
